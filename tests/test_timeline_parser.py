...
def _full_timeline() -> dict:
    ...
    return {
        ...
        "defaults": {
            "video": {"model": "veo-3.1", "duration": 8},
...
class TestDefaults:
    def test_defaults_applied_to_video_source(self):
        d = _minimal_timeline()
        d["defaults"] = {"video": {"model": "veo-3.1", "duration": 8}}
        tl = parse_timeline(d)
        vid = tl.tracks[0].clips[0].source
        assert isinstance(vid, VideoSource)
        assert vid.model == "veo-3.1"
    ...
    def test_clip_overrides_defaults(self):
        ...
        d = _minimal_timeline()
        d["defaults"] = {"video": {"model": "veo-3.1", "duration": 8}}
        d["tracks"][0]["clips"][0]["source"]["model"] = "kling-v3"
        d["tracks"][0]["clips"][0]["source"]["duration"] = 4
        tl = parse_timeline(d)
        vid = tl.tracks[0].clips[0].source
        assert isinstance(vid, VideoSource)
        assert vid.model == "kling-v3"
